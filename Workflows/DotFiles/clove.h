...
int strncpy_s(
	char* strDest, size_t numberOfElements, const char* strSource, size_t count)
{
...
static void __clove_check_int(const unsigned int check_mode, int expected,
	int result, __clove_test* _this)
{
	int pass_scenario = 0;
	if (check_mode == __CLOVE_ASSERT_CHECK_EQUALITY) {
		pass_scenario = expected == result;
	} else if (check_mode == __CLOVE_ASSERT_CHECK_DIFFERENCE) {
		pass_scenario = expected != result;
	}
...
static void __clove_check_bool(
	const unsigned int check_mode, int result, __clove_test* _this)
{
	/*
		In C standard:
		- 0 is FALSE
...
static void __clove_check_null(
	const unsigned int check_mode, void* result, __clove_test* _this)
{
	int pass_scenario;
	if (check_mode == __CLOVE_ASSERT_CHECK_EQUALITY)
...
static void __clove_check_ptr(const unsigned int check_mode, void* expected,
	void* result, __clove_test* _this)
{
	int pass_scenario;
	if (check_mode == __CLOVE_ASSERT_CHECK_EQUALITY)
		pass_scenario = expected == result;
...
static void __clove_check_float(const unsigned int check_mode, float expected,
	float result, __clove_test* _this)
{
	int pass_scenario = 0;
	if (check_mode == __CLOVE_ASSERT_CHECK_EQUALITY) {
		pass_scenario = fabsf(expected - result) <= 0.000001f;
	} else if (check_mode == __CLOVE_ASSERT_CHECK_DIFFERENCE) {
		pass_scenario = fabsf(expected - result) > 0.000001f;
	}
...
static void __clove_pad_right(char* result, char* strToPad)
{
	...
	sprintf(
		result, "%s%*.*s", strToPad, padLen, padLen, padding); // LEFT Padding
}
...
{
	printf(
		"%s Executing Test Runner in 'Fail Safe Verbose' mode\n", __CLOVE_INFO);
	...
	for (int i = 0; i < numOfTests; i++) {
		...
		char result[__CLOVE_STRING_LENGTH],
			strToPad[__CLOVE_TEST_ENTRY_LENGTH + 10];
		snprintf(strToPad, __CLOVE_TEST_ENTRY_LENGTH + 10, "%d) %s", i + 1,
			each->name);
		...
		case __CLOVE_TEST_FAILED: {
			failed++;
			printf("%s %s%s => %s@%d: %s\n", __CLOVE_ERRO, result,
				__CLOVE_FAILED, each->file_name, each->line, each->err_msg);
			break;
		}
	...
	printf("%s Total: %d, Passed: %d, Failed: %d, Skipped: %d\n", __CLOVE_INFO,
		numOfTests, passed, failed, skipped);
	if (passed == numOfTests) {
		printf("%s Test result => SUCCESS :-)\n", __CLOVE_INFO);
	} else if (failed > 0) {
		printf("%s Test result => FAILURE :_(\n", __CLOVE_ERRO);
	} else if (skipped > 0) {
		printf("%s Test result => OK, but some test has been skipped!\n",
			__CLOVE_WARN);
	}
	return;
}
...
#define __CLOVE_TEST_GUARD                                                     \
	...
	strcpy_s(                                                                  \
		_this->file_name, __CLOVE_STRING_LENGTH, __clove_rel_src(__FILE__));   \
	_this->line = __LINE__;
...
#define CLOVE_RUNNER(...)                                                      \
	...
	{                                                                          \
		...
		__clove_test* tests                                                    \
			= (__clove_test*)calloc(testSize, sizeof(__clove_test));           \
		char functs_as_str[] = #__VA_ARGS__;                                   \
		for (int i = 0; i < testSize; i++) {                                   \
			char* token;                                                       \
			char* context;                                                     \
			if (i == 0) {                                                      \
				token = strtok_s(functs_as_str, ", ", &context);               \
			} else {                                                           \
